...
The per-module descriptions maintained by the team live in the repo README ([`### comp (component)`](../../../../README.md#comp-component)) — read it first; this file adds only what goes beyond it.

## Module map

- **`org.iets3.components.core`** — the base components language: the **components chunk** (`ComponentsChunk`; the generic `Chunk` concept it extends is contributed by the mbeddr platform, `com.mbeddr.core.base`), component declarations, behavior, attributes, analysis, and expression integration (virtual folders `chunk`, `components`, `behavior`, `attributes`, `analysis`, `expr`). Note: the structure still carries a `_old` virtual folder — check deprecation status before building on anything in it (`structure.deprecation-window` in `mps-developer`).
