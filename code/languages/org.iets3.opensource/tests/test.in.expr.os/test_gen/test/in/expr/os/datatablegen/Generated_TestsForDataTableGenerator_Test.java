package test.in.expr.os.datatablegen;

/*Generated by MPS */

import java.math.BigInteger;
import java.util.List;
import jetbrains.mps.internal.collections.runtime.ListSequence;
import java.util.ArrayList;
import org.junit.Test;
import org.junit.Assert;
import org.iets3.core.expr.genjava.tests.rt.rt.EqualsTestOp;

/**
 * 
 */
public class Generated_TestsForDataTableGenerator_Test {

  public static class Table_Table_Entry {
    public final Number val1;
    public final Number val2;
    public Table_Table_Entry(Number val1, Number val2) {
      this.val1 = val1;
      this.val2 = val2;
    }
    public Object get(int colIndex) {
      switch (colIndex) {
        case 0:
          return this.val1;
        case 1:
          return this.val2;
      }
      return null;
    }
    public boolean is(Table_Table_Entry... entries) {
      for (Table_Table_Entry e : entries) {
        if (e.equals(this)) {
          return true;
        }
      }
      return false;
    }
  }
  public static class Table_Table {
    public static final Table_Table_Entry keyA = new Table_Table_Entry(new BigInteger("1"), new BigInteger("2"));
    public static final Table_Table_Entry keyB = new Table_Table_Entry(new BigInteger("3"), new BigInteger("4"));
    private static List<Table_Table_Entry> entries = ListSequence.fromList(new ArrayList<Table_Table_Entry>());
    static {
      ListSequence.fromList(entries).addElement(keyA);
      ListSequence.fromList(entries).addElement(keyB);
    }
    public static Table_Table_Entry lookup(int col, Object value) {
      for (Table_Table_Entry e : ListSequence.fromList(entries)) {
        Object vv = e.get(col);
        if (value == null && vv == null) {
          return e;
        }
        if (vv != null && vv.equals(value)) {
          return e;
        }
      }
      return null;
    }
  }
  public static final Table_Table_Entry aRow_ = Table_Table.keyA;
  public static final Number number1_ = Table_Table.keyA.val1;
  @Test
  public void Tests_item0_7503962471789840464() throws Throwable {
    Assert.assertTrue("Expected: " + new BigInteger("1") + ", but was: " + Table_Table.keyA.val1, EqualsTestOp.matches(new BigInteger("1"), Table_Table.keyA.val1));
  }
  @Test
  public void Tests_item1_7503962471791916850() throws Throwable {
    Assert.assertEquals("Expected: " + true + ", but was: " + Generated_TestsForDataTableGenerator_Test.aRow_.is(Table_Table.keyA), (Object) true, (Object) Generated_TestsForDataTableGenerator_Test.aRow_.is(Table_Table.keyA));
  }
  @Test
  public void Tests_item2_7503962471791947803() throws Throwable {
    Assert.assertEquals("Expected: " + true + ", but was: " + Generated_TestsForDataTableGenerator_Test.aRow_.is(Table_Table.keyA, Table_Table.keyB), (Object) true, (Object) Generated_TestsForDataTableGenerator_Test.aRow_.is(Table_Table.keyA, Table_Table.keyB));
  }
  @Test
  public void Tests_item3_7503962471791497471() throws Throwable {
    Assert.assertTrue("Expected: " + new BigInteger("4") + ", but was: " + Table_Table.lookup(0, new BigInteger("3")).val2, EqualsTestOp.matches(new BigInteger("4"), Table_Table.lookup(0, new BigInteger("3")).val2));
  }




}
