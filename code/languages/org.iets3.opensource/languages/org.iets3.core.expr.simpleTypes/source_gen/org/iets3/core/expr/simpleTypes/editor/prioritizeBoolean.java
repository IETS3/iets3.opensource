...
public class prioritizeBoolean implements EditorMenuItemCustomizer {
  ...
  public void customize(EditorMenuItemStyle customization, EditorMenuItemCustomizationContext context) {
    ...
    prioritizeBooleanSpecific customizer = new prioritizeBooleanSpecific();
    if (customizer.matches(context)) {
      customizer.customize(customization, context);
    }
  }

  private static class prioritizeBooleanSpecific implements EditorMenuItemCustomizer {

    public boolean matches(EditorMenuItemCustomizationContext context) {
      return new EditorMenuItemCreatingConceptContextMatcher(CONCEPTS.BooleanLiteral$$j).matchesContext(context) && getCompletionItemInformation(context) != null;
    }
    ...
    private void customize_(SNode parentNode, SNode currentChild, SContainmentLink containmentLink, EditorMenuItemStyle style, CompletionItemInformation itemInformation) {
      PriorisationHelper.prioritizeByConcept(itemInformation, style, parentNode, currentChild);
    }
  }
