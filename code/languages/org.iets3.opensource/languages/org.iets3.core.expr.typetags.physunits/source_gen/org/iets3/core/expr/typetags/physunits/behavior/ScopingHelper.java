...
public class ScopingHelper {
  ...
  public static Iterable<SNode> getVisibleUnitsFrom(SNode node) {
    ...
    // reference to the unit-less unit specified in the SIUnits accessory model
    ...
    SNode context = SNodeOperations.getNodeAncestor(node, CONCEPTS.IVisibleElementProvider$$O, true, false);
    for (SNode unit : Sequence.fromIterable(IVisibleElementProvider__BehaviorDescriptor.visibleContentsOfTypeAsSequence_id3g6LnlWuSo8.invoke(context, CONCEPTS.IUnit$tw))) {
    ...
    return (canUseNoUnit ? units : SetSequence.fromSet(units).where((it) -> !(Objects.equals(SPropertyOperations.getString(it, PROPS.name$MnvL), SPropertyOperations.getString(unitLessUnit, PROPS.name$MnvL)))));
  }

  public static Iterable<SNode> getVisibleQuantityFrom(SNode node) {
    SNode context = SNodeOperations.getNodeAncestor(node, CONCEPTS.IVisibleElementProvider$$O, true, false);
    return SNodeOperations.ofConcept(IVisibleElementProvider__BehaviorDescriptor.visibleContentsOfTypeAsSequence_id3g6LnlWuSo8.invoke(context, CONCEPTS.Quantity$W4), CONCEPTS.Quantity$W4);
  }
