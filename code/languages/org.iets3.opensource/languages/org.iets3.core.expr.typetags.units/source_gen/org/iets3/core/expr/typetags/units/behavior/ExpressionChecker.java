...
public class ExpressionChecker {
  ...
  public static boolean isSelfReferencingSpecifier(SNode specifier) {
    ...
    for (SNode convertExpression : ListSequence.fromList(SNodeOperations.getNodeDescendants(SLinkOperations.getTarget(specifier, LINKS.expression$jxzQ), CONCEPTS.ConvertExpression$wD, true, new SAbstractConcept[]{}))) {
      if (SLinkOperations.getTarget(convertExpression, LINKS.conversionSpecifier$WY3n) != null) {
        QueueSequence.fromQueue(queue).addLastElement(SLinkOperations.getTarget(convertExpression, LINKS.conversionSpecifier$WY3n));
      }
    }
    ...
    while (QueueSequence.fromQueue(queue).isNotEmpty()) {
      ...
      for (SNode convertExpression : ListSequence.fromList(SNodeOperations.getNodeDescendants(SLinkOperations.getTarget(head, LINKS.expression$jxzQ), CONCEPTS.ConvertExpression$wD, true, new SAbstractConcept[]{}))) {
        if (SLinkOperations.getTarget(convertExpression, LINKS.conversionSpecifier$WY3n) != null && !(SetSequence.fromSet(seenSpecifiers).contains(SLinkOperations.getTarget(convertExpression, LINKS.conversionSpecifier$WY3n)))) {
          QueueSequence.fromQueue(queue).addLastElement(SLinkOperations.getTarget(convertExpression, LINKS.conversionSpecifier$WY3n));
        }
      }
    }
  ...
  public static boolean hasDivExpressionBeforeMulExpression(SNode expression) {
    ...
    while (QueueSequence.fromQueue(expressions).isNotEmpty()) {
      ...
      } else if (SNodeOperations.isInstanceOf(head, CONCEPTS.ConvertExpression$wD)) {
        QueueSequence.fromQueue(expressions).addLastElement(SLinkOperations.getTarget(SNodeOperations.cast(head, CONCEPTS.ConvertExpression$wD), LINKS.expr$CW3E));
      }
    }
  ...
  public static boolean hasExpression(SNode expression, SAbstractConcept expressionConcept) {
    ...
    } else if (SNodeOperations.isInstanceOf(expression, CONCEPTS.ConvertExpression$wD)) {
      return hasExpression(SLinkOperations.getTarget(SNodeOperations.cast(expression, CONCEPTS.ConvertExpression$wD), LINKS.expr$CW3E), expressionConcept);
    } else {
      return false;
    }
  }
