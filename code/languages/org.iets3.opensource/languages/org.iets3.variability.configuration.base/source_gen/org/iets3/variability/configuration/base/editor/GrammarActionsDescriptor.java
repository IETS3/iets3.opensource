...
public class GrammarActionsDescriptor extends AbstractGrammarActionDescriptor implements IGrammarActionsDescriptor {
  ...
  public List<SubstituteMenuItem> getActions(final SubstituteMenuContext _context, final SAbstractConcept expectedOutputConceptExactly) {
    ...
    _context.getEditorMenuTrace().setDescriptor(new EditorMenuDescriptorBase("org.iets3.variability.configuration.base.editor.GrammarActionsDescriptor.getActions", null));
    try {
      ListSequence.fromList(result).addSequence(Sequence.fromIterable(new Object() {
        public Iterable<SubstituteMenuItem> query() {
          ...
          {
            final SConcept outputConcept = CONCEPTS.FeatureModelConfiguration$nE;
            ...
            if (SConceptOperations.isExactly(SNodeOperations.asSConcept(outputConcept), SNodeOperations.asSConcept(expectedOutputConceptExactly))) {
              ...
              for (final SAbstractConcept subconcept : GrammarCellsUtil.getVisibleSubconceptsNonAbstract(outputConcept, _context.getModel(), FeatureModelConfiguration_Editor.class, _context.getEditorContext())) {
                ...
                applicable &= new Object() {
                  public boolean query() {
                    return EditorCustomizationConfigHelper.getConfig().isFlagCellSubstitutionActivated(EditorCustomizationConfigHelper.getIdentifier(CONCEPTS.FeatureModelConfiguration$nE, PROPS.abstract$Wu4W), subconcept, substitutedNode, parentNode, editorContext);
                  }
                }.query();
                if (applicable) {
                  ListSequence.fromList(result).addElement(new FlagSubstituteMenuItem(parentNode, _context.getCurrentTargetNode(), subconcept, "abstract", _context, new DefaultFlagModelAccess(PROPS.abstract$Wu4W)) {
                    @Nullable
                    @Override
  ...
  protected List<TransformationMenuItem> doGetSideTransformActions(final TransformationMenuContext _context) {
    ...
    _context.getEditorMenuTrace().pushTraceInfo();
    _context.getEditorMenuTrace().setDescriptor(new EditorMenuDescriptorBase("org.iets3.variability.configuration.base.editor.GrammarActionsDescriptor.doGetSideTransformActions", null));
    try {
    } finally {
      _context.getEditorMenuTrace().popTraceInfo();
    }
