...
public class CommonBaseUtil {
   ...
   */
  public static void setAttributes(AssignmentValueList faas) {
    faas.forEach((expr, faa) -> {
      setValueIfChanged(faa, attachUnitToExpression(expr, SLinkOperations.getTarget(faa, LINKS.attribute$J5jI)));
  ...
  /**
   * Write the value only when it actually differs from the one already present, so that an
