...
public class ConfigUpdateHelper {
  ...
  public static void runSolverSyncIfNotIgnored(SNode config) {
    if (!(shouldIgnoreSolver(config))) {
      ISolvable__BehaviorDescriptor.runSolver_id7QODtLw3SMH.invoke(config);
    }
  }

  public static boolean shouldIgnoreSolver(SNode config) {
    return (new IAttributeDescriptor.NodeAttribute(CONCEPTS.SolveControl$Vn).get(config) != null) && SEnumOperations.isMember(SPropertyOperations.getEnum(new IAttributeDescriptor.NodeAttribute(CONCEPTS.SolveControl$Vn).get(config), PROPS.mode$kQof), 0x3a7ea77800f377a4L);
  }
