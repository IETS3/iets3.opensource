...
public final class AbstractEnumInTarget__BehaviorDescriptor extends BaseBHDescriptor {
  ...
  /*package*/ static SNode reduce_id4L5R3LmDzwi(@NotNull final SNode __thisNode__) {
    List<SNode> sels = Sequence.fromIterable(AbstractEnumInTarget__BehaviorDescriptor.effectiveSelectors_id48Zi1gznqdX.invoke(__thisNode__)).toList();
    if (ListSequence.fromList(sels).isEmpty()) {
      return null;
    }
    ...
    SNode start = AbstractEnumInTarget__BehaviorDescriptor.single_id3wraVjnnEfa.invoke(__thisNode__, ctx, SLinkOperations.getTarget(ListSequence.fromList(sels).first(), LINKS.literal$XL8B));
    SNode logicalExpr = ListSequence.fromList(sels).tail(ListSequence.fromList(sels).count() - 1).foldLeft(start, (s, it) -> AbstractEnumInTarget__BehaviorDescriptor.compose_id4L5R3LmDwOh.invoke(__thisNode__, SLinkOperations.getTarget(it, LINKS.literal$XL8B), s, ctx));

    return createParensExpression_30sz0_a6a3(logicalExpr);
  }
  ...
  private static SNode createParensExpression_30sz0_a6a3(SNode p0) {
