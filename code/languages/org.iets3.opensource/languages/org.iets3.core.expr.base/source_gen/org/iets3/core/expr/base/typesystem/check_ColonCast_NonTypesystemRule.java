...
public class check_ColonCast_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode cc, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    ...
    ListSequence.fromList(types).addElement(SNodeOperations.as(TypecheckingFacade.getFromContext().getTypeOf(SLinkOperations.getTarget(cc, LINKS.type$EhVN)), CONCEPTS.Type$WK));
    ListSequence.fromList(types).addElement(SNodeOperations.as(TypecheckingFacade.getFromContext().getTypeOf(SLinkOperations.getTarget(cc, LINKS.expr$CW3E)), CONCEPTS.Type$WK));
      ...
      if (SNodeOperations.isInstanceOf(errorType, CONCEPTS.RuntimeErrorType$3c)) {
        {
          ...
          IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(cc, "The cast is not allowed: " + SPropertyOperations.getString(errorType, PROPS.errorText$leWQ), "r:80cf2246-750c-4158-9056-a619ebcf894c(org.iets3.core.expr.base.typesystem)", "3723661587931256985", null, errorTarget);
        }
      }
    }
  }
