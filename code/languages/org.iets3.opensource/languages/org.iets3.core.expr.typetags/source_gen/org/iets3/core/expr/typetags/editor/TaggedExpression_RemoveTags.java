...
public class TaggedExpression_RemoveTags {
  ...
  /*package*/ static AbstractCellAction createAction_DELETE(final SNode node) {
    return new AbstractCellAction() {
      ...
      public void execute_internal(EditorContext editorContext, SNode node) {
        SNode taggedExpr = SNodeOperations.getNodeAncestor(node, CONCEPTS.TaggedExpression$jU, false, false);

        if (SLinkOperations.getChildren(taggedExpr, LINKS.tags$Lx_i) == null || ListSequence.fromList(SLinkOperations.getChildren(taggedExpr, LINKS.tags$Lx_i)).isEmpty() || (ListSequence.fromList(SLinkOperations.getChildren(taggedExpr, LINKS.tags$Lx_i)).count() == 1 && ListSequence.fromList(SNodeOperations.getNodeDescendants(ListSequence.fromList(SLinkOperations.getChildren(taggedExpr, LINKS.tags$Lx_i)).first(), null, false, new SAbstractConcept[]{})).any((it) -> SNodeOperations.getConcept(it).isAbstract()))) {
          SNodeOperations.replaceWithAnother(taggedExpr, SLinkOperations.getTarget(taggedExpr, LINKS.expr$CW3E));
        } else {
