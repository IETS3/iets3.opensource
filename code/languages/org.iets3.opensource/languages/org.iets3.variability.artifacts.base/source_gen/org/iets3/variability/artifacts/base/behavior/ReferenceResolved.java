...
public class ReferenceResolved {
  ...
  public static Iterable<ReferenceResolved> forReferencesOf(final SNode contextIVAA, SNode n) {
    return ListSequence.fromList(SNodeOperations.getReferences(n)).translate((it) -> forReference(contextIVAA, it));
  }

  public static Iterable<ReferenceResolved> forReference(SNode contextIVAA, final SReference ref) {
    // check if the reference target node is a pivot
    SNode refTarget = ref.getTargetNode();
    Optional<PivotHierarchy> pivotMaybe = IVariabilityAwareArtifact__BehaviorDescriptor.getInstantiatedSubtree3_id1C4m4CTficZ.invoke(contextIVAA, refTarget);
    if (pivotMaybe.isEmpty()) {
      // normal reference (without instantiation)
      return Collections.singleton(new ReferenceResolved(ref));
    } else {
      PivotData representative = pivotMaybe.get().representative();
      return CollectionSequence.fromCollection(pivotMaybe.get().implementations()).select((pd) -> {
        // reference to a pivot node, add relative path (only one segment)
        return new ReferenceResolved(ref, pd.pivot().getTargetNode(), pd.pivot().asSegment());
      }).union(SetSequence.fromSet(Collections.singleton(new ReferenceResolved(ref, representative.pivot().getTargetNode(), representative.pivot().asSegment()))));
    }
  }
  ...
  public ReferenceResolved(SReference ref) {
    // this uses the actual target node of the reference as target  
  ...
  public String toString() {
    return "RefResolved(" + ref.getSourceNode().getPresentation() + ", " + SNodeOperations.present(target) + ", " + relativePath + ")";
  }
}
