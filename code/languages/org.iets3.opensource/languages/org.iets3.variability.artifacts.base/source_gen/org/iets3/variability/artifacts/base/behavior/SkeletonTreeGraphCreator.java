...
public class SkeletonTreeGraphCreator implements ISkeletonVisitor {
  ...
  public boolean visit(SkeletonNode sn) {
    ...
    if (sn.isInstance()) {
      label += "InstTarget: " + check_i3mtk4_a0a0a6a11(sn.target());
    }
    ArtifactPath path = sn.getFullPath();
    String pathStr = check_i3mtk4_a0i0l(path);
    ...
    if (ListSequence.fromList(segments).isNotEmpty()) {
      label += "\\npivot: " + IterableUtils.join(ListSequence.fromList(segments).select((it) -> it.getName()), ".") + " / " + check_i3mtk4_a0a0a21a11(sn.targetIVAA());
    }
    ...
    boolean hasVarPoints = sn.hasVarPoint() || sn.getTabular() != null;
  ...
  private static String check_i3mtk4_a0a0a6a11(SNode checkedDotOperand) {
    if (null != checkedDotOperand) {
      return SNodeOperations.present(checkedDotOperand);
    }
    return null;
  }
  private static String check_i3mtk4_a0i0l(ArtifactPath checkedDotOperand) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.asSegmentPathString();
    }
    return null;
  }
  private static String check_i3mtk4_a0a0a21a11(SNode checkedDotOperand) {
    if (null != checkedDotOperand) {
