...
public class FeatureDecTabContent_ActionMap {
  ...
  /*package*/ static AbstractCellAction createAction_DELETE(final SNode node) {
    return new AbstractCellAction() {
      ...
      public void execute_internal(EditorContext editorContext, SNode node) {
        int nItems = ListSequence.fromList(SLinkOperations.getChildren(SNodeOperations.cast(SNodeOperations.getParent(node), CONCEPTS.FeatureDecTab$p6), LINKS.contents$oelO)).count();
        if (nItems > 1) {
          // do usual two-step approve/delete logic only if this isn't the last item in the list
          if (!(DeletionApproverUtil.approve(editorContext, node))) {
            SNodeOperations.deleteNode(node);
          } else {
            // will not delete this node if it is the only item in the parent's list
          }
        }
      }
