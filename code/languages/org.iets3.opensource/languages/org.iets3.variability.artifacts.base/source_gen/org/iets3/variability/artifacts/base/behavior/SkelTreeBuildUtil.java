...
public class SkelTreeBuildUtil {
   ...
   * @param nonStructuralChildren nodes related to 'n', but not structurally contained (content from inheritance, for example)
   * @return all direct and logical children, excluding the skip-node (if any)
   */
  public List<SNode> relevantChildren(SNode n, final Iterable<SNode> toBeSkipped, List<? extends SNode> nonStructuralChildren) {
    // start with actual AST children (but exclude the toBeSkipped-nodes, if any)
    List<SNode> children = Lists.newLinkedList(ListSequence.fromList(SNodeOperations.getChildren(n)).union(ListSequence.fromList(nonStructuralChildren)).where((it) -> !(Sequence.fromIterable(toBeSkipped).contains(it))));
  ...
  public List<SNode> relevantChildren(SNode n, Iterable<SNode> toBeSkipped) {
    return this.relevantChildren(n, toBeSkipped, Collections.<SNode>emptyList());
  }
