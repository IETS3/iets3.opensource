...
public class DefaultTraceExplorerActionHandler extends AbstractTraceExplorerAction implements ITraceExplorerAction {
  ...
  /*package*/ void runTracer(String title, Icon icon, ITraceRecord root) {
    ...
    final JComponent tab = ideaProject.getComponent(ProjectPluginManager.class).getTool(TraceExplorer_Tool.class).openTrace(traceTabOptionFactory.createTraceTabOption(ideaProject, mpsProject, title, icon, root));
    // The cast below is actually valid in the generated code so the error is suppressed.
    final BaseTabbedProjectTool tool = (BaseTabbedProjectTool) (ideaProject.getComponent(ProjectPluginManager.class).getTool(TraceExplorer_Tool.class));
