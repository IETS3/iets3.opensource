...
- Core: results of manual checks are no longer cleared by the checking rule that reports them. That clearing was hidden while a run was a single command, but with checks running one at a time the checker ran in between and results vanished before the run was over. Results now stay until the next manual run or until the user clears them with the new actions `Clear Manual Check Results in Root` (editor popup) and `Clear Manual Check Results in Model` (model popup), or the existing `Delete Stored Results` intention. Clearing a KernelF test item's result also removes its red highlight, and clearing a result that was stored before a language reload no longer throws a `ClassCastException`.
- Core: while a bulk manual run is active, the editors are refreshed during the run (at most every 500 ms) and not only at its end, and queued items show `queued` or `running` in place of `no result found`. The KernelF test item editors and the REPL editor show this state.
- Core: items that write the model but need no UI can opt in to `canRunManuallyInWriteAction()` and are then run in a write action on the background thread instead of a command on the EDT. The remaining EDT items are run in slices of about 100 ms with a short pause in between, so the status bar and the editor can paint and keystrokes get through; back-to-back EDT commands used to starve painting.
- Core: the progress of a bulk manual run appears in the status bar right away. The first intermediate editor refresh no longer happens at the start but after the first interval, and the interval adapts to the duration of the previous refresh, so a big root is not rebuilt on the EDT every 500 ms. The indicator shows `Collecting N checks` while the items are being classified.
