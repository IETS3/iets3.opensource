...
## September 2026
### Added
- KernelF: a new generator language `org.iets3.core.expr.genjava.optimizer` post-processes the generated Java for readability, as a separate step of the `Java_genplan` generation plans between the KernelF generators and the `closures`/`collections`/`unitTest` generators. It inlines immediately invoked closures (`{ => return e; }.invoke()` becomes `e`, `if`/`else` returns become a ternary, bodies in return/statement/variable-initializer position are spliced into the surrounding block, and a closure that only returns at the end of its `if`/`try` branches initialises its variable by assignments in those branches; `T v = e; return v;` becomes `return e;`), collapses `T v = init; v = v.a(); …; return v;` closures into `init.a()…` call chains, folds `if (true)`/`true ? …` conditions, and removes redundant casts (nested casts to the same type, and casts whose operand already has the type or a subtype of it, where the static type cannot change overload resolution or type inference) and redundant parentheses, and replaces anonymous classes over functional interfaces (`Function`, `Predicate`, `Comparator`, …) as well as the remaining immediately invoked closures by Java lambdas (`(x) -> …`, `((Supplier<T>) () -> { … }).get()`), so that the generated code no longer depends on the MPS closures runtime for these; and it flattens the bare blocks that loop macros leave behind, inlines the temporaries declared in them and turns the `StringBuilder` closures of string interpolation into plain `+` concatenations. The KernelF generators themselves are unchanged and keep describing the semantics only.
